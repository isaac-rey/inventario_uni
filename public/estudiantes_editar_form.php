...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($ci === '' || $nombre === '' || $apellido === '' || $email === '') {
    ...
    $stmt = $mysqli->prepare("SELECT id FROM estudiantes WHERE (ci=? OR email=?) AND id<>? LIMIT 1");
    $stmt->bind_param('ssi', $ci, $email, $id);
    $stmt->execute();
    $existe = $stmt->get_result()->fetch_assoc();

    if ($existe) {
      $error = 'Ya existe otro estudiante con ese CI o Email.';
    } else {
      if ($pass !== '') {
        $hash = password_hash($pass, PASSWORD_BCRYPT);
        $stmt = $mysqli->prepare("UPDATE estudiantes SET ci=?, nombre=?, apellido=?, email=?, password_hash=? WHERE id=?");
  ...
  <div class="card">
    ...
    <?php else: ?>
      ...
      <form method="post" autocomplete="off">
        <label>CI</label>
        <input name="ci" type="text" required value="<?= htmlspecialchars($estudiante['ci']) ?>">

        <label>Nombre</label>
        <input name="nombre" type="text" required value="<?= htmlspecialchars($estudiante['nombre']) ?>">

        <label>Apellido</label>
        <input name="apellido" type="text" required value="<?= htmlspecialchars($estudiante['apellido']) ?>">

        <label>Email</label>
        <input name="email" type="email" required value="<?= htmlspecialchars($estudiante['email']) ?>">
