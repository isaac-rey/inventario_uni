...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['prestamo_id'], $_POST['motivo'])) {
    ...
    //-------------Inserssion de auditoría de cancelacion------------
    auditar("Solicitó la cancelación del préstamo ID {$prestamo_id}. Motivo: '{$motivo}'");
    //---------------------------------
...
if ($rol === 'admin' || $rol === 'titular_area') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['aprobar_id'])) {
        ...
        //-------------Insersion de auditoría de aprobacion------------
        auditar("Aprobó la cancelación del préstamo ID {$id}. (Solicitante: {$solicitante}). Equipo liberado.");
        //---------------------------------
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['rechazar_id'])) {
        ...
        //---------------Insersion de Auditoría de Rechazo------------
        auditar("Rechazó la cancelación del préstamo ID {$id}. (Solicitante: {$solicitante}).");
        //---------------------------------
